...
<?php $title = "Ganti Pasword - Spades Discord" ?>
...
?>
	<div class="container">
		<div class="content">
			...
			<p>Ganti password mahasiswa dengan Id Mahasiswa <?php echo '<b>'.$_GET['id_discord'].'</b>'; ?></p> 
			...
			if(isset($_POST['ganti'])){ 
				...
				$cek = mysqli_query($koneksi, "SELECT * FROM tbl_discord WHERE id_discord='$id_discord' AND password='$password'");
				if(mysqli_num_rows($cek) == 0){
					...
					if($password1 == $password2){
						if(strlen($password1) >= 4){
							$pass = md5($password1);
							$update = mysqli_query($koneksi, "UPDATE tbl_discord SET password='$pass' WHERE id_discord='$id_discord'");
							if($update){
