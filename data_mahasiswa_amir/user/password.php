...
<?php $title = "Ganti Pasword - Data Member" ?>
...
?>
	<div class="container">
		<div class="content">
			...
			$sql = mysqli_query($koneksi, "SELECT * FROM tbl_discord WHERE username='$username'"); // query memilih entri username pada database
			...
			if(isset($_POST['ganti'])){ // jika tombol 'Simpan' dengan properti name="ganti" ditekan
				...
				$cek = mysqli_query($koneksi, "SELECT * FROM tbl_discord WHERE username='$username' AND password='$password'"); // query memilih username dan password
				if(mysqli_num_rows($cek) == 0){ // mengecek query $cek jika password yang dimasukkan tidak sesuai dengan username
					...
					if($password1 == $password2){ // jika nilai password1 dan password2 bernilai sama
						if(strlen($password1) >= 4){ // mengecek panjang password minimal 4 karakter
							...
							$update = mysqli_query($koneksi, "UPDATE tbl_discord SET password='$pass' WHERE username='$username'"); // query update password dari username yang dipilih
			...
			<form class="form-horizontal" action="" method="post">
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						...
						<a href="profile.php" class="btn btn-sm btn-danger" data-toggle="tooltip" title="Batal"><b>Batal</b></a>
					</div>
				</div>
			</form>
		</div> <!-- /.content -->
